...
class TransformerModel(nn.Module):
    def __init__(self, input_dim, d_model, output_dim):
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=1, dim_feedforward=128, batch_first=True, norm_first=True)
    ...
    def forward(self, src):
        ...
        class_tokens = self.class_token.expand(src.size(0), -1, -1)
        src = torch.cat((class_tokens, src), dim=1)
        src = self.pos_encoder(src)
        transformer_output = self.encoder(src)
        output = self.fc_out(transformer_output[:, 0, :])
        return output
...
# 定义损失函数和优化器
criterion = nn.BCEWithLogitsLoss(pos_weight = torch.full((1, 20), 4)).to(device)  # 用于多标签分类的损失函数
